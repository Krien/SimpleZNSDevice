...
int main(int argc, char **argv) {
  ...
  for (auto it = devices_available.begin(); it != devices_available.end();
       it++) {
  ...
  printf("\tAVG Reset time %f \n",
         static_cast<double>(ti) / static_cast<double>(resets));
  uint64_t variance = std::sqrt(static_cast<double>(tisq * resets - ti * ti) /
                                static_cast<double>(resets * resets));
